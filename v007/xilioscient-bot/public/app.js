...
(function($) {
    ...
    class XSBot {
		...
		const defaultBanner = '/wp-content/plugins/xilioscient-bot/xsbot-assets/banner.jpg';
		const defaultProfile = '/wp-content/plugins/xilioscient-bot/xsbot-assets/propic.png';

            const bannerUrl = (typeof botConfig.bannerImage !== 'undefined' && botConfig.bannerImage) ? botConfig.bannerImage : defaultBanner;
            const profileUrl = (typeof botConfig.profileImage !== 'undefined' && botConfig.profileImage) ? botConfig.profileImage : defaultProfile;
            ...
            try {
                const response = await fetch(botConfig.restUrl + 'message', {
                    ...
                    headers: { 
                        ...
                        'X-WP-Nonce': botConfig.nonce 
                    },
                    body: JSON.stringify({ message: message, session_id: this.sessionId })
                });

                // Leggiamo il testo prima di decidere se è JSON
                const responseText = await response.text();

                if (!response.ok) {
                    // Qui loggeremo il contenuto dell'errore 500
                    console.error("Errore Server (500):", responseText);
                    throw new Error(`Errore Server ${response.status}: Il server ha risposto con un errore.`);
                }

                // Se è andata bene, convertiamo in JSON
