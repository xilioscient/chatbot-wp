...
class XSBOT_LLM_Proxy {
    ...
    private static function send_to_gemini($message, $api_key) {
        ...
        $body = array(
            'contents' => array(
                array(
                    'role' => 'user',
                    'parts' => array(
                        array('text' => "Sei un assistente AI utile e cortese. Rispondi in italiano.\n\nDomanda: " . $message)
                    )
                )
            ),
        ...
        // Struttura della risposta di Gemini: candidates[0].content.parts[0].text
        ...
        return new WP_Error('gemini_error', __('Errore Google Gemini', 'xilioscient-bot'));
    }
